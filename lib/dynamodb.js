...
DynamoDB.prototype.all = function all(model, filter, cb) {
  ...
  if (properties) {
    // TODO primary index logic is mostly broken
    for (var i = 0; i < properties.length; i++) {
      // try the primary index first
      if (!me.isInPrimaryIndex(properties[i], model)) {
    ...
    if (!whereIsIndexed) {
      index = me.findSecondaryIndex(properties, order && order[0], model);
      if (index) {
        whereIsIndexed = true;
      }
    }
...
DynamoDB.prototype.findSecondaryIndex = function (properties, orderBy, model) {
  if (this.dataSource.definitions[model] &&
      this.dataSource.definitions[model].settings.indexes) {
      var indexes = this.dataSource.definitions[model].settings.indexes;
      var bestIndex = null;
      var bestCount = 0;
...
DynamoDB.prototype.addConditionToParam = function (params, index, key, conditionValue, operator) {
  ...
  function isIndexed(key, operator, index) {
    var useIndex = false;
    // if key == index.rangeKey and operator in [ eq, lt, lte, gt, gte, between]
    // OR if key == index.hashKey and operator is eq
      // see if rangeKey and supported operator
    if (index.rangeKey.key === key &&
      ['lt', 'lte', 'gt', 'gte', 'between'].indexOf(operator) != -1) {
      useIndex = true;
    } else if (index.hashKey.key === key || index.rangeKey.key === key) {
      useIndex = (operator === 'eq');
    }
    return useIndex;
  }
  ...
  if (Object.prototype.toString.call(conditionValue) == "[object Object]") {
    ...
    operators.forEach(function (keyValue) {
      var useIndex = isIndexed(key, keyValue, index);
      switch (keyValue) {
      case 'inq':
